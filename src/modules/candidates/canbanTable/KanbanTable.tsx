...
type SetUpdatedUserType = React.Dispatch<React.SetStateAction<Candidate | null>>;
...
function KanbanTable() {
  ...
  useEffect(() => {
    CandidatesSerivce.candidateHttpPost('GetList', CandidatePost).then(response => {
      ...
      Object.keys(agg).forEach(item => {
        for (let i = 0; i < candidates.length; i++) {
          if (item == candidates[i].status) {
            agg[item] = agg[item] ? [...agg[item], candidates[i]] : [candidates[i]];
  ...
  return (
    <DragDropContext
      onDragEnd={result => onDragEnd(result, aggregatedData, setAggregatedData, setUpdatedUser, setDestinationColumn)}>
      <TableWrapper>
